...
generator.buildProductions = function buildProductions (bnf, productions, nonterminals, symbols, operators) {
  ...
  function buildProduction(handle) {
    ...
    if (handle.constructor === Array) {
      ...
      if (typeof handle[1] === 'string' || handle.length === 3) {
        ...
        if (action.match(/[$@][a-zA-Z][a-zA-Z0-9_]*/)) {
          ...
          action = action.replace(/\$([a-zA-Z][a-zA-Z0-9_]*)/g, function (_, pl) {
            return names[pl] ? '$' + names[pl] : _;
          }).replace(/@([a-zA-Z][a-zA-Z0-9_]*)/g, function (_, pl) {
            return names[pl] ? '@' + names[pl] : _;
          });
        }
        ...
        rhs = rhs.map(e => e.replace(aliasRegex, ''));
        r = new Production(symbol, rhs, productions.length + 1);
  
        if (handle[2] && typeof handle[2] === 'object' && handle[2].prec && operators[handle[2].prec]) {
        ...
        if (handle[1] && typeof handle[1] === 'object' && handle[1].prec && operators[handle[1].prec]) {
    ...
    // fallback: infer precedence from handle tokens
    if (r.precedence === 0) {
